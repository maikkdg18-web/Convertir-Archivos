// Supabase Edge Function: convierte PDFs a DOCX usando CloudConvert.
...
serve(async (req) => {
  ...
  try {
    ...
    if (!fileUrl || !fileName) {
      return jsonResponse({ success: false, error: "Se necesita un PDF válido." }, 400);
    }
    ...
    return jsonResponse({
      success: false,
      error: err instanceof Error ? err.message : "Error desconocido al convertir el PDF.",
    }, 500);
  }
});
